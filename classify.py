...
a1=time.ctime(os.path.getmtime(lst[0]))    
b1=timecal(a1)
a2=time.ctime(os.path.getmtime(lst[1]))    
b2=timecal(a2)
...
for i in range(1, int(len(lst)/2)+1) :
            if(2*i+1==(int(len(lst)))):
                n1=time.ctime(os.path.getmtime(lst[2*i]))    
                n0=time.ctime(os.path.getmtime(lst[2*i-1]))
                t1=timecal(n1)
                t0=timecal(n0)
                ...
                n1=time.ctime(os.path.getmtime(lst[2*i]))    
                n2=time.ctime(os.path.getmtime(lst[2*i+1]))
                n0=time.ctime(os.path.getmtime(lst[2*i-1]))
                t2=timecal(n2)
                t1=timecal(n1)
                t0=timecal(n0)
                if ((t1-t0)<=args.time_long):
                    if ((t2-t1)<=args.time_long):
                        ...
                        shutil.copy('{:}'.format(lst[2*i]),"({:})".format(n)) 
                        log("time {:}".format(n1))
                        log("Keep copy {:} to folder ({:})".format(lst[2*i],n))
